...
class Appointment(namedtuple('Appointment', 'calendarId status subStatus emailAddress fullName dateOfBirth, printed')):
    ...
    def from_dict(cls, d: dict):
        apt = {
            'calendarId': safe_get(d, 'calendarId'),
            'status': safe_get(d, 'status'),
            'subStatus': safe_get(d, 'subStatus'),
            'emailAddress': safe_get(d, 'client', 'emailAddress'),
            'fullName': safe_get(d, 'client', 'fullName'),
            'dateOfBirth': Appointment.convert_dateOfBirth(safe_get(d, 'client', 'dateOfBirth')),
        }
    ...
    while True:
        ...
        if event == 'Sync' or event == sg.TIMEOUT_KEY:
            ...
            for a in appointments:
                if safe_get(a, 'location', 'locationId') != location:
                    continue
                if (cid := safe_get(a, 'calendarId')) is None:  # Get calendarId
                    print('calendarId error, not processing')
                    continue
                cursor.execute(
